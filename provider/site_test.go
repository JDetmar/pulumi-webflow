...
// NOTE: TestSiteDiff_ShortNameReadOnly was removed because shortName is now
// an output-only field (not in SiteArgs). Users can no longer provide shortName
// as an input, so there's nothing to diff.
...
func TestSiteDrift_ShortNameFromAPI_ShouldNotTriggerChange(t *testing.T) {
	...
	// API always returns shortName, so Read() populates it in State as output-only
	stateFromRead := SiteState{
		SiteArgs: SiteArgs{
			WorkspaceID: "workspace456",
			DisplayName: "My Test Site",
		},
		ShortName: "my-test-site", // API returned this, Read() included it as output
	}
