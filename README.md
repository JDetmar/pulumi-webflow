# Pulumi Native Provider Webflow
...
This boilerplate creates a working Pulumi-owned provider named `pulumi-webflow`.
...
1. Set the following options:
   * Owner: pulumi 
   * Repository name: pulumi-webflow (replace "pulumi-webflow" with the name of your provider)
...
1. Run the following command to update files to use the name of your provider (third-party: use your GitHub organization/username):
   ...
   - find and replace all instances of `pulumi-webflow` with the `NAME` of your provider.
   - find and replace all instances of the boilerplate `abc` with the `ORG` of your provider.
   - replace all instances of the `github.com/jdetmar/pulumi-webflow` repository with the `REPOSITORY` location
...
```bash
$ cd examples/simple
$ yarn link @jdetmar/pulumi-webflow
$ yarn install
...
1. A `provider/` folder containing the building and implementation logic.
    1. `cmd/pulumi-resource-pulumi-webflow/main.go` - holds the provider's sample implementation logic.
