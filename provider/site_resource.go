...
// This file defines the Site resource schema and stubs for CRUD operations.
// Full CRUD implementation is provided in Story 3.2: Site Creation Operations and beyond.
package provider
...
// Site is the resource controller for managing Webflow sites.
...
func (args *SiteArgs) Annotate(a infer.Annotator) {
	...
	a.Describe(&args.Publish,
		"Automatically publish the site after creation or updates. "+
			...
			"Default: false (manual publishing required). "+
			"Note: Site must have at least one published version before automatic publishing will work. "+
			"If publishing fails, the entire operation will fail with an error (site may exist but Pulumi will report failure). "+
...
func (r *SiteResource) Create(ctx context.Context, req infer.CreateRequest[SiteArgs]) (infer.CreateResponse[SiteState], error) {
	...
	if req.DryRun {
		// Return preview state with preview ID that matches Webflow format (24-char hex)
		// Use timestamp + padding to create a valid-looking preview ID
		previewId := fmt.Sprintf("%024x", time.Now().Unix())
	...
	response, err := PostSite(ctx, client, req.Inputs.WorkspaceId, req.Inputs.DisplayName, req.Inputs.ParentFolderId, req.Inputs.TemplateName)
	if err != nil {
...
func (r *SiteResource) Read(ctx context.Context, req infer.ReadRequest[SiteArgs, SiteState]) (infer.ReadResponse[SiteArgs, SiteState], error) {
