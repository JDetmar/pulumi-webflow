...
// TestSiteCreate_ShortNameSetViaPatch verifies that when a user provides a shortName,
// the Create function issues a follow-up PATCH to set it, since the Webflow Create Site
// API does not accept shortName in the request body.
func TestSiteCreate_ShortNameSetViaPatch(t *testing.T) {
	postCalled := false
	patchCalled := false

	// Mock POST /v2/workspaces/{workspaceId}/sites - Create site (no shortName)
	postServer := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		response := Site{
			...
			WorkspaceID: "ws-123",
			DisplayName: reqBody.Name,
			ShortName:   "my-custom-site", // auto-generated
		}
		w.WriteHeader(201)
		_ = json.NewEncoder(w).Encode(response)
	}))
	...
	// Mock PATCH /v2/sites/{siteId} - Update shortName
	patchServer := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		_ = json.Unmarshal(body, &reqBody)
		// displayName should be empty (not sent)
		if reqBody.DisplayName != "" {
			t.Errorf("Expected empty displayName in PATCH, got '%s'", reqBody.DisplayName)
		}
		if reqBody.ShortName != "custom-slug" {
			t.Errorf("Expected shortName 'custom-slug', got '%s'", reqBody.ShortName)
		}
		response := Site{
			ID:          "site-abc",
			WorkspaceID: "ws-123",
			DisplayName: "My Custom Site",
			ShortName:   "custom-slug", // user's requested shortName
		}
		w.WriteHeader(200)
		_ = json.NewEncoder(w).Encode(response)
	}))
	...
	oldPatchURL := patchSiteBaseURL
	postSiteBaseURL = postServer.URL
	patchSiteBaseURL = patchServer.URL
	defer func() {
		postSiteBaseURL = oldPostURL
		patchSiteBaseURL = oldPatchURL
	}()

	// Set token via env var for GetHTTPClient
	...
	if !postCalled {
		t.Error("Expected POST to be called for site creation")
	}
	if !patchCalled {
		t.Error("Expected PATCH to be called to set shortName")
	}
	if resp.Output.ShortName != "custom-slug" {
		t.Errorf("Expected shortName 'custom-slug', got '%s'", resp.Output.ShortName)
	}
	if resp.ID != "site-abc" {
...
// TestSiteCreate_NoShortName_NoPatch verifies that when no shortName is provided,
// the Create function does NOT issue a follow-up PATCH and uses the auto-generated shortName.
func TestSiteCreate_NoShortName_NoPatch(t *testing.T) {
	patchCalled := false
	...
	if patchCalled {
		t.Error("PATCH should NOT be called when shortName is not provided")
	}
...
// TestSiteDiff_ShortNameChanged tests shortName change alone
func TestSiteDiff_ShortNameChanged(t *testing.T) {
	resource := &SiteResource{}
	req := infer.DiffRequest[SiteArgs, SiteState]{
		Inputs: SiteArgs{
			WorkspaceID: "workspace456",
	...
	if _, ok := diff.DetailedDiff["shortName"]; !ok {
		t.Error("Expected 'shortName' in DetailedDiff")
	}
	if len(diff.DetailedDiff) != 1 {
		t.Errorf("Expected only 1 change (shortName), got %d", len(diff.DetailedDiff))
	}
}
