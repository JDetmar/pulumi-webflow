...
func (args *SiteArgs) Annotate(a infer.Annotator) {
	...
	a.Describe(&args.ShortName,
		"DEPRECATED: This field is ignored. shortName is read-only and auto-generated "+
			"by Webflow from the displayName. The Webflow API does not support setting "+
			"shortName on create or update. The actual shortName value is available as "+
			"a read-only output. This input field is preserved for backwards compatibility "+
			"with existing stacks and will be removed in a future major release.")
	a.Deprecate(&args.ShortName, "shortName is read-only and auto-generated by Webflow. "+
		"Remove it from your Pulumi program inputs.")
...
func (r *SiteResource) Update(
	...
	// shortName is deprecated input but effectively read-only (auto-generated by Webflow);
	// preserve from old state when user doesn't specify it.
	if state.ShortName == "" {
		state.ShortName = req.State.ShortName
	}
